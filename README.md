# Regulatory Frameworks in Credit Risk
...
What the bank can do here is to hold out capital against the loan it gives out. For instance, if it is going out a loan to a person the bank deems risky, then the bank must have the same (or almost the same) amount of cash with it, in case the person does default. The more risky the party whom the bank is lending money, the more capital they will have to hold against it.

Since people's hard earned money was involved, this needed to be regulated — that is, the need to demarcate rigorously how much capital had to be held for a given level of risk profile. For that, we had the Basel Accords.
...
This expected loss is provisioned for in the income statement, but the market fluctuates and the tail risks can be significantly different from what is expected. The capital allocated as a safety net thus shields against such unexpected losses.
